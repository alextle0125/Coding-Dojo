...
export class CrimeComponent implements OnInit {
  	...
  	d3.json("https://cdn.jsdelivr.net/npm/us-atlas@2/us/10m.json").then(data => {
	  	...
	  	d3.csv("https://gist.githubusercontent.com/mbostock/682b782da9e1448e6eaac00bb3d3cd9d/raw/0e0a145ded8b1672701dc8b2a702e51c648312d4/unemployment.csv").then(countyData => {
	  		for(let i = 0; i < countyData.length; i++){
	  			this.dataObj.push({ id: countyData[i].id, state: countyData[i].state, county: countyData[i].county, crimes: 0 });
	  		}
		  	d3.csv("https://gist.githubusercontent.com/alextle0125/ed6c75a20b40b2ced3893f1e2ea4fa21/raw/3a7ce73001a28dc1450411f17ed6fdc4e89ab22d/Sheet3-Table%25201").then(crimeData => {
		  		for(var i = 0; i < this.dataObj.length; i++){
		  			for(var j = 0; j < crimeData.length; j++){	
			  			if(this.dataObj[i]['id']===crimeData[j]['STCOU']){
			  				this.dataObj[i]['crimes'] = ((crimeData[j][yearDataDict[this.year]] / crimeData[0][yearDataDict[this.year]]) * 100).toPrecision(3);
			  				break;
			  			} else {
			  				continue;
			  			}
  ...
  chart(us, data) {
  	...
  	const color = d3.scaleQuantize([0, 5], d3.schemeReds[9]);
	const states = new Map(us.objects.states.geometries.map(d => [d.id, d.properties]));
	this.mapData = new Map(data.map(d => [d.id, +d.crimes]));
